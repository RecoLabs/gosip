...
func (items *Items) GetByCAML(ctx context.Context, caml string) (ItemsResp, error) {
	...
	client := NewHTTPClient(items.client)
	return client.Post(ctx, apiURL.String(), bytes.NewBuffer(body), items.config)
}
...
func getAll(ctx context.Context, res []ItemResp, cur ItemsResp, items *Items) ([]ItemResp, error) {
	if res == nil && cur == nil {
		...
		itemsResp, err := itemsCopy.Get(ctx)
		if err != nil {
			return nil, err
		}
		res = itemsResp.Data()
	...
	nextItemsResp, err := NewItems(items.client, nextURL, items.config).Get(ctx)
	if err != nil {
		return res, err
	}
	res = append(res, nextItemsResp.Data()...)
	return getAll(ctx, res, nextItemsResp, items)
}
...
func (items *Items) GetPaged(ctx context.Context) (*ItemsPage, error) {
	data, err := items.Get(ctx)
	if err != nil {
		return nil, err
	}
	res := &ItemsPage{
		...
		GetNextPage: func() (*ItemsPage, error) {
			...
			return NewItems(items.client, nextURL, items.config).GetPaged(ctx)
		},
	}
	return res, nil
}
